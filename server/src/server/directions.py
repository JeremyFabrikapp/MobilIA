    ...
    async def get_journey_from_verbal_points(self, start: str, finish: str, datetime_str: str, wheelchair: bool = False):
        ...
        try:
            ...
            if journey_info is None:
                raise ValueError(
                    "Unable to fetch journey information from the API")
            ...
            return {"error": "An unexpected error occurred while fetching journey information"}
                    ...
                    for link in display_info.get('links', []):
                        if link.get('type') == 'disruption':
                            ...
                            matching_disruption = next(
                                (d for d in disruptions if d['id'] == disruption_id), None)
